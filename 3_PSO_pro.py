...
device = 'cuda:0'
dataset = Dataset('./data_processing/data/train.txt', 70)
model_generator = RNN_VAE(vocab_size=paramters.vocab_size, max_seq_len=56, device=device, **paramters.model).to(device)
model_generator.load_state_dict(torch.load('./output/wae/model_epoch_397_6.pt', map_location=torch.device(device)))


model_classifier = Model_classifier(device, './output/wae/model_epoch_397_6.pt').to(device)
state = torch.load('./output/classifier/uptest/uptestclassifier_uptest_300.tar', map_location=torch.device(device))
...
def fit_fun(z):
    ...
    for i in range(len(seq)):        
        ...
        if '<pad>' in seq[i] or '<unk>' in seq[i]:
            ...
            data = prepare_dataset(np.array(one), 70)
...
class PSO():
    ...
    def run(self, max_iter, N):
        ...
        for iter_num in pbar:
            ...
            self.collected_Y = self.collected_Y + list(self.Y)
            self.update_pbest()
            self.update_gbest()
            self.gbest_y_hist.append(self.gbest_y)
        self.best_x, self.best_y = self.gbest_x, self.gbest_y
        return self.collected, self.gbest_y_hist, self.collected_Y
...
class MyThread(Thread):
    ...
    def run(self):
        self.collected, _,self.collected_Y = self.func(*self.args)
...
for thread in threads:
    ...
    result_all += res
    res_prob = thread.get_result_prob()
    result_all_prob += res_prob


with open('./output/PSO_samples/66/output_66.json','w') as f:
    json.dump(list(set(result_all)),f)

with open('./output/PSO_samples/66/output_66_prob.txt','w') as f_prob:
    for seq_prob in result_all_prob:
        line = str(seq_prob).replace('\n',',')
        f_prob.write(line)
        f_prob.write('\n')