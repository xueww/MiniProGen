...
class PeptideEvaluator():
    ...
    def similarity(self, seqs_lst1, seqs_lst2):
        ...
        #print(sim_lst)
        with open('x.txt', 'a') as note:
            for element in sim_lst:
                note.write(str(element)+'\n')
        av_sim = sum(sim_lst) / len(sim_lst) if len(sim_lst) > 0 else 0.0

        return resultsdict, av_sim
#################  test 

# petide = PeptideEvaluator(seq_len=35)
# petide_list = ['ETFSDWWKLLAE']
# a = petide.similarity_my(petide_list, ['ETFADWWKLLAE'],matrix_size=1)
# petide.calculate_charge('ETFSDWWKLLAE')