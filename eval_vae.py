...
dataset = Dataset('./data_processing/data/test.txt', 70)
device = 'cuda:0'
model = RNN_VAE(vocab_size=paramters.vocab_size, max_seq_len=65, device=device, 
                **paramters.model).to(device)
pytorch_total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
evaluate = True
if evaluate:
    state = torch.load('./output/wae/model_epoch_392.pt', map_location=torch.device(device))
