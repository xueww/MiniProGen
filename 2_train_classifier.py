...
device = 'cuda:0'
##data
with open('./data_processing/data/classifier_uptest.json') as f:
    data = json.load(f)
train_dataset = Dataset_classifier(data['train'], 70)
test_dataset = Dataset_classifier(data['test'], 70)
...
model_class = Model_classifier(device, './output/wae/model_epoch_397_6.pt').to(device)
...
for epoch in range(1, 300+1):
    ...
    if epoch % 1 == 0:
        torch.save(model_class.state_dict(), './output/classifier/uptest' + 'classifier_uptest_%s.tar'%str(epoch))